 ...
 * 
 * Monitors and manages Firebase connection state, providing reconnection
 * functionality when network issues occur.
 */
 ...
 * This sets up listeners for Firebase connection state changes
 */
export function initializeConnectionMonitor(): void {
  if (connectionMonitorInitialized || typeof window === 'undefined' || !app) return;
...
function updateLastSeen(): void {
  ...
  try {
    const db = getDatabase(app);
    const lastSeenRef = ref(db, `client-status/${getUserIdentifier()}`);
    set(lastSeenRef, { 
 ...
 * @returns Boolean indicating whether Firebase is connected
 */
export function isFirebaseConnected(): boolean {
  return isConnected;
}

// Auto-initialize on import if we're on the client side
