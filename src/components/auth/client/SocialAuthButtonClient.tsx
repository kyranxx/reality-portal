...
import { SocialAuthButtonProps } from '../SocialAuthButton';

interface ClientSocialAuthButtonProps extends SocialAuthButtonProps {
  onClick?: () => void;
}

/**
 * Client-side button component for social authentication providers.
 * This component includes event handler logic and is marked with 'use client'
 * to ensure it only runs on the client side.
 */
export function SocialAuthButtonClient({
  provider,
  onClick,
  isLoading = false,
  disabled = false,
}: ClientSocialAuthButtonProps) {
