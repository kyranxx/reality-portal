...
function validateEnvironmentVariables() {
  ...
  if (allGood) {
    ...
    console.log(
      `${colors.yellow}⚠ Some environment variables are missing or appear to be placeholders${colors.reset}`
    );
  }
...
function validateFirebaseRules() {
  ...
  try {
    ...
    if (openBraces !== closeBraces) {
      console.log(
        `${colors.red}✖ Firebase rules have mismatched braces: ${openBraces} opening vs ${closeBraces} closing${colors.reset}`
      );
      results.errors++;
      return false;
    }
    ...
    const patterns = {
      ...
      'collection security':
        rulesContent.includes('match /') && rulesContent.includes('/documents/'),
    };
...
function validateFirebaseService() {
  ...
  try {
    ...
    console.log(
      `${colors.red}✖ Error validating Firebase service: ${error.message}${colors.reset}`
    );
    results.errors++;
    return false;
  }
